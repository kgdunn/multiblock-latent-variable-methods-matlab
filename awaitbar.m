...
if nargin>=2              
    if ischar(whichbar)            % h =awaitbar(0,'Please wait...');
        ...
        if strcmp(whichbar, '<<unique string>>')
            f = findobj('Tag', 'TMWAWaitbar');
            set(f, 'Visible', 'on');
            return
        end
        ...
        set(0, 'showhid', 'on');
        try delete(findobj('Tag', 'TMWAWaitbar')); 
        catch ME
        end
...
% Initialize			
        ...
        %action = sprintf('awaitbar(-1.0,  ''<<unique string>>'')');		
		% Create and start the timer to delay for 4 seconds.
		%start(timer('TimerFcn', action, 'StartDelay', 5.0));
        
		
		% Abort button
		uicontrol('Style', 'pushbutton', 'String', 'Stop',...
			'FontWeight','bold',...
			'FontSize',11,'units','Points','Position', [0.8.*width  3 50 25],...
		...
		uicontrol('Parent',f,...
			'Units','points',...
			'Position',[pos(3).*0.05 pos(4).*0.25 pos(3).*0.45 pos(4).*0.17],...
			'String','Elapsed:',...
		...
		uicontrol('Parent',f,...
			'Units','points',...
			'Position',[pos(3).*0.05 pos(4).*0.05 pos(3).*0.45 pos(4).*0.17],...
			'String','Remaining:',...
			'BackgroundColor', [0.80 0.80 0.80],...
            'HorizontalAlignment', 'left', ...
			'Style','text');
...
% UPDATE OF WAITBAR
	case 2, 
		try            
			...
			if isempty(f) || isempty(p) || isempty(l) || isempty(lapseObj)||isempty(etaObj)
